...
class Connector extends \Emergence\Connectors\AbstractConnector
{
    ...
    public static function handleLinkUserRequest()
    {
        ...
        setcookie('gh-state', $stateToken, time() + 600, '/connectors/meetup');

        Site::redirect('https://secure.meetup.com/oauth2/authorize', [
            'client_id' => API::$clientId,
            'redirect_uri' => (Site::getConfig('ssl') ? 'https' : 'http').'://'.$_SERVER['HTTP_HOST'].'/connectors/meetup/oauth/link-user',
            'scope' => 'user repo',
            'state' => $stateToken
        ]);
    }
    ...
    public static function handleUnlinkUserRequest()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] != 'POST') {
            return static::respond('confirm', [
                'question' => _('Are you sure you want to unlink your Meetup user account?')
            ]);
        }

        $GLOBALS['Session']->Person->MeetupToken = null;
        $GLOBALS['Session']->Person->save();

        return static::respond('message', [
            'message' => 'Your Meetup account has been unlinked from your user profile successfully'
        ]);
    }
    ...
    public static function handleOAuthRequest()
    {
        ...
        if (empty($_COOKIE['meetup-state']) || $_GET['state'] != $_COOKIE['meetup-state']) {
            return static::throwInvalidRequestError('The returning Meetup state token does not match the one you were sent out with, please try again');
        }
        ...
        setcookie('gh-state', '', time() - 3600, '/connectors/meetup');

        $responseData = API::request('https://secure.meetup.com/oauth2/access', [
            'skipAuth' => true,
            'headers' => [
        ...
        if (empty($responseData['access_token'])) {
            throw new Exception('access_token not returned by Meetup');
        }

        if (static::peekPath() == 'link-user') {
            $GLOBALS['Session']->Person->MeetupToken = $responseData['access_token'];
            $GLOBALS['Session']->Person->save();

            return static::respond('message', [
                'message' => 'Your Meetup account has been linked to your user profile successfully'
            ]);
        }

        return static::respond('message', [
            'message' => 'Access token issued by Meetup: ' . $responseData['access_token']
        ]);
    }
}